...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  if (t_cur < traj_duration_ && t_cur >= 0.0) {
    ...
    ROS_WARN_THROTTLE(2, "flight time: %lf, path length: %lf, mean vel: %lf, energy is: % lf ", flight_t, len, len / flight_t, energy);
  } else {
...
int main(int argc, char** argv) {
  ...
  ros::Timer vis_timer = node.createTimer(ros::Duration(0.01), visCallback);
