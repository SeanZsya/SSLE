...
Eigen::Vector3d state_pos(0,0,0);
Eigen::Vector3d state_vel(0,0,0);
Eigen::Vector3d state_acc(0,0,0);
    ...
    while(ros::ok())
    {
        ...
        case prometheus_msgs::ControlCommand::Move:
            ...
            if(Command_Now.Reference_State.Move_frame  == prometheus_msgs::PositionReference::ENU_FRAME)
            {
                ...
                //NEW REFERENCE STATE: XYZ_VA, deploying both velocity and acceleratioin 
                else if ( Command_Now.Reference_State.Move_mode  == prometheus_msgs::PositionReference::XYZ_VA )
                {
                    state_vel = Eigen::Vector3d(Command_Now.Reference_State.velocity_ref[0],Command_Now.Reference_State.velocity_ref[1],Command_Now.Reference_State.velocity_ref[2]);

                    state_acc = Eigen::Vector3d(Command_Now.Reference_State.acceleration_ref[0],Command_Now.Reference_State.acceleration_ref[1],Command_Now.Reference_State.acceleration_ref[2]);

                    yaw_sp = Command_Now.Reference_State.yaw_ref;
                }
                //NEW REFERENCE STATE: XYZ_ALL, deploying pos, vel, acc 

                 else if ( Command_Now.Reference_State.Move_mode  == prometheus_msgs::PositionReference::XYZ_ALL )
                {   
                    state_pos = Eigen::Vector3d(Command_Now.Reference_State.position_ref[0],Command_Now.Reference_State.position_ref[1],Command_Now.Reference_State.position_ref[2]);
            ...
            else if( Command_Now.Reference_State.Move_mode  == prometheus_msgs::PositionReference::XYZ_VA )
            {
                ...
                {
                    yaw_rate_sp = Command_Now.Reference_State.yaw_rate_ref;
                    _command_to_mavros.send_vel_acc_setpoint_yaw_rate(state_vel,state_acc, yaw_rate_sp);
                }else
                {
                    _command_to_mavros.send_vel_acc_setpoint(state_vel,state_acc, yaw_sp);
                }
            ...
            else if( Command_Now.Reference_State.Move_mode  == prometheus_msgs::PositionReference::XYZ_ALL )
            {
                ...
                {
                    yaw_rate_sp = Command_Now.Reference_State.yaw_rate_ref;
                    _command_to_mavros.send_all_setpoint_yaw_rate(state_pos,state_vel,state_acc, yaw_rate_sp);
                }else
                {
                    _command_to_mavros.send_all_setpoint(state_pos,state_vel,state_acc, yaw_sp);
                }
            }
            //====================================================================
