...
int main(int argc, char **argv)
{
    ...
    //【发布】位置控制器的输出量:期望姿态
    att_ref_pub = nh.advertise<prometheus_msgs::AttitudeReference>(uav_name + "/prometheus/control/attitude_reference", 10);      
        
    //【发布】参考位姿 RVIZ显示用
    ref_pose_pub = nh.advertise<geometry_msgs::PoseStamped>(uav_name + "/prometheus/control/ref_pose_rviz", 10);

    // 【发布】用于地面站显示的提示消息
    message_pub = nh.advertise<prometheus_msgs::Message>(uav_name + "/prometheus/message/main", 10);

    // 【发布】用于log的消息
    log_message_pub = nh.advertise<prometheus_msgs::LogMessage>(uav_name + "/prometheus/topic_for_log", 10);
